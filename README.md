...
**适用于 Minecraft 基岩版命令 IDE**  
一个功能完整、支持离线使用的 Minecraft 命令集成开发环境，完全基于 Godot 4 构建。

MCfun 提供了从项目创建、代码编辑、实时解析与错误检测，到行为包导出的完整工作流。其底层的语法引擎由可热加载的 JSON 定义语法系统驱动，支持最新的 Minecraft 命令（beta 1.21），且无需修改核心代码即可扩展。
