...
## 如果字符串是这个类型，返回 [code]true[/code]。
static func is_type(text : String, type : GrammerValue.Type) -> bool:
	match type:
		GrammerValue.Type.NIL:
			return text.is_empty()
		GrammerValue.Type.INT:
			return text.is_valid_int()
		GrammerValue.Type.FLOAT:
			return text.is_valid_float()
		GrammerValue.Type.SCOPE:
			return ScopeElement.is_valid_scope(text)
		GrammerValue.Type.STRING:
			return not text.is_empty()
		GrammerValue.Type.SELECTOR:
			return SelectorElement.is_selector(text)
		GrammerValue.Type.COORD:
			return CoordElement.is_coord(text)
		GrammerValue.Type.QUOTATION:
			return text.begins_with("\"") and text.ends_with("\"") and not text.ends_with("\\\"")
		GrammerValue.Type.DICTIONARY:
			return text.begins_with("{") and text.ends_with("}") and not text.ends_with("\\{")
...
static func try_get_type(text : String, offset : int) -> Array[GrammerValue.Type]:
	...
	if StrT.is_letter_char_ord(valid_str.unicode_at(0)):
		return [GrammerValue.Type.WORD, GrammerValue.Type.POINT_PATH, GrammerValue.Type.SELECTOR, GrammerValue.Type.SPACEITEM]
