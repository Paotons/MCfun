...
## 可作为虚函数，创建。
static func create(text : String, offset : int, rule : ElementRule = null) -> StringElement:
	return _create_string_element(StringElement.new(), text, offset, rule)
## [b]friend [BaseStringElement]:[/b] 可用于创建特定类型的结果。
static func _create_string_element(element : BaseStringElement, text : String, offset : int, rule : ElementRule = null) -> BaseStringElement:
	element.string_offset = offset
	
	var result := _string_searched_regex.search(text.substr(offset))
	if result != null:
		element.string = text.substr(offset) if rule != null and rule.is_long() else result.get_string()
