...
## CMD列表。
var cmd_list : Dictionary[String, Dictionary]
...
## 高亮数据。
var hightlight_data : HightLightData
...
static func create(text : String, offset : int, line := -1) -> CommandElement:
	...
	element.hightlight_data = HightLightData.new()
	
	# 进程。
	var process :=_CreateProcess.new()
	
	process.edit = GL.edit
	process.grammer = GL.get_grammer()
...
func get_hightlight(_edit : FunctionEdit) -> Dictionary[int, Dictionary]:
	return hightlight_data.data
...
static func _update_code_completion_head_data() -> void:
	var data := CodeCompletionData.new()
	data.insert_texts.append_array(GL.get_grammer().get_heads())
...
func is_valid_head() -> bool:
	return GL.get_grammer().has_head(head_string)
...
## 获取指令列表。
func get_cmd_list(name : String, column := -1) -> PackedStringArray:
	var result : PackedStringArray
	if _has_child_element:
		for child in get_children_element(true):
			result.append_array(child.get_cmd_list(name, column))
	if not cmd_list.has(name): return result
	column = 0xFFFFFFFF if column == -1 else column
	var list : Dictionary = cmd_list[name]
	for key : int in list:
...
func get_faild_element(idx : int) -> ExeElementRule:
	return GL.get_grammer().get_item(head_string, faild_element_idxs[idx])
## 返回失败的执行元素的数量。
...
func _do_head(text : String, process : _CreateProcess) -> bool:
	...
	hightlight_data.merge(result.get_hightlight(process.edit))
	head_element = result
	head_string = head
...
func _do_default(text : String, process : _CreateProcess) -> bool:
	...
	hightlight_data.merge(element.get_hightlight(process.edit))
