...
## 规则。
var grammer_rule : GrammerParamBacketRule
## 变量类型。
var value_type := -1
...
func _get_hightlight(edit : FunctionEdit) -> Dictionary[int, Dictionary]:
	...
	if value_element != null:
		if value_element is StringElement:
			result.merge(value_element.get_hightlight(edit), true)
	else:
		super(edit)
	return result
...
static func create(text : String, offset : int, start := "\"", end := "\"", rule : GrammerParamBacketRule = null) -> ParamBacketElement:
