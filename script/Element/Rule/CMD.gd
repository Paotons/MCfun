...
static func _execute_list(cmd : Array, element : StringElement, command : CommandElement) -> void:
	...
	var list_name : String = cmd[2]
	var mode : int = cmd[1]
	if mode == _ItemListMode.ADD: # 添加
		if not command.cmd_list.has(list_name): command.cmd_list[list_name] = {}
		command.cmd_list[list_name][element.get_valid_start()] = element.get_valid_string()
## 执行补全，并把补全的数据返回。
static func execute_completion(column : int, rule : ElementRule, command : CommandElement) -> CodeCompletionData:
	...
	for cmd : Array in rule.get_cmd():
		...
		if mode == _ItemCompletionMode.LIST:
			var list_name : String = cmd[2]
			data.insert_texts.append_array(edit.get_command_cmd_list(list_name, command.get_line_index(), column - 1))
	return data
#endregion
