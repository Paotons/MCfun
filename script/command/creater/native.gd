...
func run_from_empty(text : String, process : CommandElementCreaterProcess) -> void:
	...
	_get_hl_data().merge({process.offset : {"color" : process.edit.color_native_command_head}, process.offset + 1 : {"color" : process.edit.color_default}})
	process.offset += 1
...
func _do_head(text : String, process : CommandElementCreaterProcess) -> bool:
	var result := HeadElement.create(text, process.offset, CommandElementManager.CommandType.NATIVE) as HeadElement
