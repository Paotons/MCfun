...
## 关键字颜色。
@export var color_key_world := Color("ff7085")
## 数字颜色。
...
func _ready() -> void:
	...
	#region 初始化语法。
	assert(grammer_json != null, "Grammer is null.")
	assert(grammer_law_json != null, "Grammer runle is null.")
	assert(grammer_entry_json != null, "Completion entry is null.")
	
	grammer = Grammer.new()
	if grammer_json != null:
		grammer.compile(grammer_json.data)
...
func find_has_error_command(from_line := 0, to_line := -1) -> int:
	...
	return -1
## 指令，查找前方指令的列表。
func get_command_cmd_list(name_ : String, line : int, column : int) -> PackedStringArray:
	var init_ele := get_command_element(line)
	var result := init_ele.get_cmd_list(name_, column) if init_ele != null else PackedStringArray()
	var hash_map : Dictionary[int, bool]
	for i in range(line - 1, maxi(-1, line - 501), -1):
		for arr : String in get_command_element(i).get_cmd_list(name_):
			var hash_ := arr.hash()
